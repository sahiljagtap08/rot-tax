...
from .scorers import score_A, score_B, score_C, score_count
...
def run_one(cell, agent_client, cost, logger, model_version):
    ...
    if probe.kind == "A":
        ...
        passed, detail = (score_count(resp.text, gold) if hard else score_B(resp.text, gold))
    else:
        passed, detail = score_C(resp.text)

    sig = extract_signals(trial.filler_turns, resp.text, resp.input_tokens)
    return {
        "session_id": sid, "item": item, "rep": rep, "content_seed": seed, "probeset": ps,
