...
\paragraph{Deeper-latent robustness (closing the 2-hop limitation).}
Because the hard retrieval probe is only 2-hop, we ran a focused $340$-trial
extension with a strictly harder latent probe: a $3$-hop routing chain
...
regime NoLiMa identifies as harshest~\cite{modarressi2025nolima}. Across all three
models tested (\texttt{gpt-5.4-mini}, \texttt{gpt-5.5}, \texttt{claude-sonnet-4-6}),
present-needle accuracy was $1.000$ at every length to 150k, with the no-needle
control again at chance ($0.000$); notably, even \texttt{gpt-5.4-mini}---the only
model to slip anywhere---solves the deeper, distractor-laden retrieval perfectly.
The null thus holds not just for shallow latent retrieval but for deeper,
interference-laden latent retrieval as well.
